...
struct TSuffixTree {
    struct TEdge {
        ...
        shared_ptr<int> Right; //Правая граница, не включая последнее, то есть -1
        int IdTo; //В какую вершину ведёт ребро

        TEdge(int l, shared_ptr<int> r, int id) : Left(l), Right(r), IdTo(id) {}
    };


    string DataString; //Text
    vector< vector< TEdge > > Data; //Списки смежности
    vector<int> SuffixPtr; //Вектор суффиксных ссылок
    vector<int> PathSize; // Вектор сколько символов от корня до текущей вершины
    ...
    TSuffixTree(const string & s) : DataString(s), Data(s.size() * 2), SuffixPtr(s.size() * 2), PathSize(s.size() * 2) {
        bool newVertex = false;
        int l = 0;
        shared_ptr<int> end(new int);
        *end = 0;
            ...
            while (l < *end) {
                ...
                if (newVertex) {
                    ...
                    if (curEq == 0 and curEdgeId == -1) {
                        ...
                        shared_ptr<int> newRightBorder(new int);
    ...
    vector<size_t> Search(const string & s) {
        size_t n = s.size();
        vector<size_t> ms(n);
        int curVertex = 0;
        ...
        for (size_t i = 0; i < n; ++i) {
            ...
            path.push(curVertex);
            curEq = max(0, passEq + curEq - 1 - PathSize[curVertex]);
...
int main() {
    ...
    vector<long unsigned int> ms(text.size());
    ms = tree.Search(text);
